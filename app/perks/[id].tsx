...
export default function PerksScreen() {
  ...
  const normalizedPerks: Perk[] = perks.map((p) => ({
    ...p,
    category:
      p.category && p.category in CATEGORY_CONFIG
        ? (p.category as Exclude<PerkCategory, null>)
        : 'other',
  }));
  ...
  const filteredPerks =
    selectedCategory === 'All'
      ? normalizedPerks
      : normalizedPerks.filter((p) => p.category === selectedCategory);

  const activePerkCount = normalizedPerks.filter((p) =>
    canRedeem(p, membership?.tier),
  ).length;
        ...
        >
          ...
          <View style={styles.heroStats}>
            <View style={styles.heroStat}>
              <Text style={styles.heroStatNum}>{normalizedPerks.length}</Text>
        ...
        <Animated.View entering={FadeInDown.delay(200).duration(400)}>
          ...
          >
            {CATEGORY_FILTERS.map((cat) => {
              ...
              const catConfig =
                cat.id !== 'All'
                  ? CATEGORY_CONFIG[cat.id as Exclude<PerkCategory, null>]
                  : null;
                  ...
                  style={[
                    ...
                    isActive && {
                      backgroundColor:
                        cat.id === 'All'
                          ? Colors.primary
                          : catConfig?.color || Colors.primary,
                      borderColor:
                        cat.id === 'All'
                          ? Colors.primary
                          : catConfig?.color || Colors.primary,
        ...
        <View style={styles.list}>
          {isLoading ? (
            ...
            filteredPerks.map((perk, i) => {
              ...
              const catKey: Exclude<PerkCategory, null> =
                perk.category && perk.category in CATEGORY_CONFIG
                  ? (perk.category as Exclude<PerkCategory, null>)
                  : 'other';
                  ...
                  >
                    ...
                    <View style={styles.perkMeta}>
                      ...
                      {perk.endDate && (
                        <View style={styles.metaTag}>
                          ...
                          />
                          <Text style={styles.metaTagText}>
                            Ends {new Date(perk.endDate).toLocaleDateString()}
                          </Text>
                        </View>
                      )}
                    ...
                    >
                      <Ionicons
                        ...
                        size={16}
                        color={redeemable ? '#FFF' : Colors.textTertiary}
                      />
                      <Text
                        style={[
