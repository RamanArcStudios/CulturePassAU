...
const CATEGORY_ROUTES: Record<ResultType, string> = {
  event: '/(tabs)/explore',
  movie: '/movies',
...
export default function SearchScreen() {
  ...
  const { data: events = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`${base}/api/events${queryString}`);
  ...
  const { data: movies = [] } = useQuery({
    ...
    queryFn: async () => {
      const base = getApiUrl();
      const res = await fetch(`${base}/api/movies${queryString}`);
      if (!res.ok) throw new Error(`${res.status}`);
      const data = await res.json();
      return Array.isArray(data) ? data : [];
    },
  ...
  const { data: restaurants = [] } = useQuery({
    ...
    queryFn: async () => {
      const base = getApiUrl();
      const res = await fetch(`${base}/api/restaurants${queryString}`);
      if (!res.ok) throw new Error(`${res.status}`);
      const data = await res.json();
      return Array.isArray(data) ? data : [];
    },
  ...
  const { data: activities = [] } = useQuery({
    ...
    queryFn: async () => {
      const base = getApiUrl();
      const res = await fetch(`${base}/api/activities${queryString}`);
      if (!res.ok) throw new Error(`${res.status}`);
      const data = await res.json();
      return Array.isArray(data) ? data : [];
    },
  ...
  const { data: shopping = [] } = useQuery({
    ...
    queryFn: async () => {
      const base = getApiUrl();
      const res = await fetch(`${base}/api/shopping${queryString}`);
      if (!res.ok) throw new Error(`${res.status}`);
      const data = await res.json();
      return Array.isArray(data) ? data : [];
    },
    ...
    events.forEach((e: any) => {
      if (
        ...
        results.push({
          ...
          type: 'event',
          title: e.title,
          subtitle: `${e.communityTag || ''} · ${e.venue || ''}`,
          imageUrl: e.imageUrl,
          icon: 'calendar',
          color: '#E85D3A',
        });
    ...
    movies.forEach((m: any) => {
      if (
        ...
        results.push({
          ...
          type: 'movie',
          title: m.title,
          subtitle: `${m.language || ''} · ${(m.genre || []).join(', ')}`,
    ...
    activities.forEach((a: any) => {
      if (
        ...
        results.push({
          ...
          type: 'activity',
          title: a.name,
          subtitle: `${a.category || ''} · ${a.priceLabel || ''}`,
          imageUrl: a.imageUrl,
          icon: 'football',
          color: '#3498DB',
        });
    ...
    communities.forEach((c: any) => {
      if (c.name?.toLowerCase().includes(q) || c.category?.toLowerCase().includes(q)) {
        results.push({
          ...
          type: 'community',
          title: c.name,
          subtitle: `${c.category || ''} · ${c.members || 0} members`,
          icon: 'people',
          color: '#16656E',
        });
      }
  ...
  const handleTypeSelect = useCallback((type: ResultType | 'all') => {
    if (!isWeb) {
      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    }
  ...
  const handleResultPress = useCallback(
    (result: SearchResult) => {
      ...
      Keyboard.dismiss();
      router.push({
        pathname: ROUTE_MAP[result.type] as any,
        params: { id: result.id },
      });
    },
    []
  ...
  const handleCategoryPress = useCallback((key: ResultType) => {
    if (!isWeb) {
      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    }
    Keyboard.dismiss();
    router.push(CATEGORY_ROUTES[key] as any);
  }, []);
        ...
        <View style={styles.searchBar}>
          ...
          {query.length > 0 && (
            <Pressable
              ...
              hitSlop={8}
              android_ripple={{ color: Colors.textTertiary + '20', radius: 12 }}
              accessibilityRole="button"
      ...
      {query.length > 0 && allResults.length > 0 && (
        ...
        >
          <Pressable
            ...
            accessibilityLabel={`All results, ${typeCounts.all} items`}
          >
            <Text
              style={[
                styles.typeChipText,
          ...
          {(Object.keys(TYPE_CONFIG) as ResultType[])
              ...
              <Pressable
                ...
                style={[
                  ...
                  selectedType === type && {
                    ...
                    borderColor: TYPE_CONFIG[type].color,
                  },
                ]}
                onPress={() => handleTypeSelect(type)}
                android_ripple={{ color: TYPE_CONFIG[type].color + '30' }}
                accessibilityRole="button"
                accessibilityState={{ selected: selectedType === type }}
                accessibilityLabel={`${TYPE_CONFIG[type].label}, ${typeCounts[type]} items`}
              >
                <Ionicons
                  name={TYPE_CONFIG[type].icon as any}
                  size={14}
                  color={selectedType === type ? '#FFF' : TYPE_CONFIG[type].color}
                />
                <Text
                  style={[
                    styles.typeChipText,
        ...
        {query.length === 0 ? (
          <View style={styles.suggestionsContainer}>
            ...
            <View style={styles.categoriesGrid}>
              {(Object.entries(TYPE_CONFIG) as [ResultType, typeof TYPE_CONFIG[ResultType]][]).map(
                ([key, config]) => (
                  <Pressable
                    key={key}
                    style={styles.categoryCard}
                    onPress={() => handleCategoryPress(key)}
                    android_ripple={{ color: config.color + '20' }}
                    accessibilityRole="button"
                    accessibilityLabel={`Browse ${config.label}`}
                  >
          ...
          <View style={styles.resultsList}>
            ...
            {filteredResults.map((result, index) => (
              <Animated.View
                ...
                entering={isWeb ? undefined : FadeInDown.delay(index * 40).duration(300)}
              >
                <Pressable
                  style={styles.resultCard}
                  onPress={() => handleResultPress(result)}
                  android_ripple={{ color: result.color + '10' }}
                  accessibilityRole="button"
                  accessibilityLabel={`${result.title}, ${TYPE_CONFIG[result.type].label}`}
                >
                  {result.imageUrl ? (
                    <Image
