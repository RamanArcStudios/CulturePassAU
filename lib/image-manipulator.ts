/**
 * Default / fallback image manipulation using Canvas API.
 * Metro resolves this file when platform-specific variants (.web.ts, .native.ts)
 * are not matched — e.g. when the bundler doesn't detect the web platform.
 * This avoids the native module resolution issues with expo-image-manipulator
 * in web-only environments (e.g. Replit).
 *
 * Platform resolution order:
 *   web    → image-manipulator.web.ts   (Canvas)
 *   native → image-manipulator.native.ts (expo-image-manipulator)
 *   other  → image-manipulator.ts        (this file, Canvas fallback)
 */
